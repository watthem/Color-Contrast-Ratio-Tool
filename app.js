...
function Color(hex, name) {
	...
	var ratio = function ratio(againstValue) {

		var againstColor = new Color(againstValue);

		var valueLuminanace = this.Luminosity();
		var againstLuminanace = againstColor.Luminosity();

		if (isNaN(valueLuminanace) || isNaN(againstLuminanace)) {
			console.log("Unable to call Color.Ratio: Luminosity was NaN (valueLuminance=" + valueLuminanace + " vs againstLuminance=" + againstLuminanace + ")");
		}
	...
	var luminosity = function luminosity() {
		var rgbFromHex = this.RGB();

		if (rgbFromHex) {

			// http://stackoverflow.com/questions/9733288/how-to-programmatically-calculate-the-contrast-ratio-between-two-colors
			var calcFrom = function calcFrom(r, g, b) {
				var rgbArray = [r, g, b].map(function(arrayValue) {
					arrayValue /= 255;
			...
			var returnValue = calcFrom(rgbFromHex.Red, rgbFromHex.Green, rgbFromHex.Blue);
	...
	Object.defineProperty(this, "Luminosity", {
		get: function get() {
			return luminosity;
		},
		set: function set(value) {
...
function rgbTohex(domValue) {
	var colorCompArray = /(.*?)rgb\((\d+), (\d+), (\d+)\)/.exec(domValue);

	var red = parseInt(colorCompArray[2]);
	var green = parseInt(colorCompArray[3]);
	var blue = parseInt(colorCompArray[4]);

	var rgb = blue | green << 8 | red << 16;
	return colorCompArray[1] + "#" + rgb.toString(16);
}

var ColorArray = function ColorArray(fromObject) {
	var array = new Array();
	fromObject.forEach(function(color) {
		return array.push(new Color(color.hex, color.name));
	});
	return array;
};

function TableRow(rowDiv, color) {
	this.rowTableTemplate = "<div class=\"row-cell primary-color-cell\"><div class=\"color-display\"><div class=\"color-ratio\">1.0</div></div><div class=\"color-name\"></div></div><div class=\"row-cell secondary-color-cell\"><div class=\"color-display\"><div class=\"color-ratio\">1.0</div></div><div class=\"color-hex\"></div></div>";

	var rowColor = this.color = color;
	this.RowDiv = rowDiv;
	var rowElement = this.RowDiv;
	var dataSetColor = function dataSetColor(newColor) {

		rowElement.dataset.color = newColor.hex;
		rowElement.dataset.name = newColor.name;
	};

	Object.defineProperty(this, "DataSetColor", {
		get: function get() {
			return dataSetColor;
		},
		set: function set(value) {
	...
	var primaryColumnBackgroundColor = function primaryColumnBackgroundColor(newColor) {
		if (rowElement) {
			rowElement.querySelector(".primary-color-cell").style.backgroundColor = newColor.hex;
		} else {
	...
	Object.defineProperty(this, "PrimaryColumnBackgroundColor", {
		get: function get() {
	...
	var secondaryColumnBackgroundColor = function secondaryColumnBackgroundColor(newColor) {
		if (rowElement) {
			rowElement.querySelector(".secondary-color-cell").style.backgroundColor = newColor.hex;
		} else {
	...
	Object.defineProperty(this, "SecondaryColumnBackgroundColor", {
		get: function get() {
	...
	var displayedColor = function displayedColor(newColor) {
		if (rowElement) {
			toArray(rowElement.querySelectorAll(".color-display")).forEach(function(display) {
				return display.style.backgroundColor = newColor.hex;
			});
		} else {
	...
	Object.defineProperty(this, "DisplayedColor", {
		get: function get() {
	...
	var colorHexValue = function colorHexValue(newColor) {
		if (rowElement) {
			var querySelector = rowElement.querySelector(".color-hex");
	...
	Object.defineProperty(this, "ColorHexValue", {
		get: function get() {
	...
	var colorNameValue = function colorNameValue(newColor) {
		if (rowElement) {
			var querySelector = rowElement.querySelector(".color-name");
	...
	Object.defineProperty(this, "ColorNameValue", {
		get: function get() {
	...
	this.ColorContrastRatioAgainst = function(newColor) {
		if (rowElement) {
			toArray(rowElement.querySelectorAll(".color-ratio")).forEach(function(colorRatio) {
				var againstColor = colorRatio.parentElement.parentElement.style.backgroundColor;
	...
	this.updateRowValues = function() {
		if (rowElement) {
			...
			rowElement.innerHTML += this.rowTableTemplate;
...
function buildColorTable(colorArray) {
	...
	colorArray.forEach(function(color) {
		...
		var tableRow = new TableRow(document.createElement("div"), color);

		targetTable.appendChild(tableRow.RowDiv);
...
function setBackground(newColor) {
	...
	toArray(document.querySelectorAll(".color-results")).forEach(function(element) {
...
function handleRatioClick(event) {
	...
	if (this.parentNode.classList.contains("primary")) {
		var oldColor = primaryColor.hex;
		var newHex = shadeColor(oldColor, amount);

		setUpdatingInput("primary", newHex);
	} else if (this.parentNode.classList.contains("secondary")) {
		var _oldColor = secondaryColor.hex;
		var _newHex = shadeColor(_oldColor, amount);
		console.log(_newHex);
		setUpdatingInput("secondary", _newHex);
	}
}

function handleFilterClick(event) {

	var a = new ColorArray(colors);
	...
	if (this.classList.contains("active")) {
		...
		var filtered = a.filter(function(x) {
			var againstValue = filteredParent.classList.contains("primary") ? primaryColor.hex : secondaryColor.hex;
			var ratio = x.Ratio(againstValue);
			if (ratio >= 3 && ratio <= 4) {
				return x;
			}
		});
...
function setRaito(element, color, against) {
	...
	var ratioSelector = element.querySelector(".color-ratio");
	if (ratioSelector) {
		var ratio = color.Ratio(against.hex);
		var sentimentColor = "";
		ratioSelector.innerText = ratio;

		var result = "sentiment_neutral";
		if (ratio <= 1) {
...
function setUpdatingInput(eventName, newColor) {
	...
	if (eventName.startsWith("primary")) {
		...
		updateColorTable();
		toArray(document.querySelectorAll(".primary")).forEach(function(element) {
		...
		updateColorTable();
		toArray(document.querySelectorAll(".secondary")).forEach(function(element) {
...
function isFilteredSoRebuild() {
	toArray(document.querySelectorAll(".color-filter")).forEach(function(element) {
...
function updateColorTable() {
	...
	toArray(targetTable.querySelectorAll(".table-row")).forEach(function(element) {

		var tableRow = new TableRow(document.createElement("div"), new Color(element.dataset.color, element.dataset.name));
		targetTable.appendChild(tableRow.RowDiv);
...
(function() {
	var colorList = new ColorArray(colors);
	buildColorTable(colorList);

	toArray(document.querySelectorAll(".color-swatch input")).forEach(function(colorInput) {
		colorInput.addEventListener("change", handleInputChange, true);
	});

	//toArray(document.querySelectorAll(".table-row"))
	//	.forEach(function(tableRow) {
	//		tableRow.addEventListener("click", handleRowClick);
	//	});

	toArray(document.querySelectorAll(".color-ratio")).forEach(function(ratio) {
